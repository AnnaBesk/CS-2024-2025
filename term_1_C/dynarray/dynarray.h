...
void resize(Dynarray* pd, size_t new_size){
    ...
    else if (new_size < pd->capacity){
        for (int i = pd->size; i < new_size; ++i){
            pd->data[i] = 0;
        }
        pd->size = new_size;
    }
...
Dynarray shallow_copy(Dynarray* pd){
    Dynarray shallow;
    shallow.size = pd->size;
    shallow.capacity = pd->capacity;
    shallow.data = pd -> data;
    return shallow;
}
...
Dynarray deep_copy(Dynarray* pd){
    ...
    shallow.size = pd->size;
    shallow.capacity = pd->capacity;
    shallow.data = (int*)ecmalloc(shallow.capacity * sizeof(int));
    
    for (int i = 0; i < shallow.size; ++i){
        shallow.data[i] = pd->data[i];
    }
