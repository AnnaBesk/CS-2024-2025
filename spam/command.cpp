...
class DeleteCommand: public Command
{
    ...
    public:
        DeleteCommand(ShapePool& pool) 
            : mPool(pool) {}
    
        void execute() override
        {
            for (auto ptr : mPool.mShapes) {
                delete ptr;  
            }
            mPool.mShapes.clear();
        }
...
class MultiCommand: public Command
{
    ...
    public:
        MultiCommand(ShapePool& pool)
        : mPool(pool), num_mCommands(0)
        {
        }
        ...
        void execute() override
        {
            for (size_t i = 0; i < num_mCommands; ++i){
                mCommands[i]->execute();
            }
        }
};
...
int main()
{
    ...
    MultiCommand* mc = new MultiCommand(pool); 
    mc->addCommand(new AllRandomColorCommand(pool));
    mc->addCommand(new RandomAllPositionsCommand(pool));
    ...
    control.addButton(new Button(window, {40, 380, 240, 40}, font, "All Random Color"), new AllRandomColorCommand(pool));
    control.addButton(new Button(window, {40, 440, 240, 40}, font, "Delete"), new DeleteCommand(pool));
    control.addButton(new Button(window, {40, 500, 240, 40}, font, "MultiCommand"), mc);
