...
class Circle_my : public Observer, public Drawable
{
protected:
    sf::CircleShape mShape;
    sf::RenderWindow& mRenderWindow;
...
public:
    Circle_my(sf::RenderWindow& window, sf::Vector2f position, float radius) 
    : mRenderWindow(window), mShape(radius)
    {
        mShape.setOrigin({radius, radius});
        mShape.setPosition(position);
    ...
    void update(float value) override
    {
        mShape.setPosition({100, 350 + value*2});
    }
};
...
int main()
{
    ...
    std::vector<Observer*> observers
    {
        ...
        new Circle_my{window, {100, 400}, 25}
    };
